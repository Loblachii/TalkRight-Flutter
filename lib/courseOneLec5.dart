...
import 'dart:async';
import 'courseOneLec5Exercise.dart';
import 'progress_manager.dart';
import 'settings_manager.dart';
import 'notification_helper.dart';

class CourseOneLec5 extends StatefulWidget {
  final bool showModal;

  const CourseOneLec5({super.key, this.showModal = false});

  @override
  State<CourseOneLec5> createState() => _CourseOneLec5State();
}

class _CourseOneLec5State extends State<CourseOneLec5>
  ...
  static const int lessonIndex = 4;
  ...
  static const List<String> _ttsPhrases = [
    "Some sounds stick together",
    "Tap to hear the sound",
    "Blend in a word",
    "Emphasized beginning blend",
  ];
  ...
  Future<void> _playOnPageLoad() async {
    ...
    if (_currentPage == 0) {
      ...
      // Page 3 — TTS → blConsonant (mid-chain) → clConsonant (last).
      // Hint fires after clConsonant completes via onPlayerComplete.
      _registerTtsHandlers(
        onComplete: () async {
          await Future.delayed(const Duration(milliseconds: 300));
          _isChainedAudio = true; // suppress hint after first m4a
          await _playLetterSound('blConsonant.m4a');
          _audioPlayer.onPlayerComplete.first.then((_) async {
            await Future.delayed(const Duration(milliseconds: 400));
            _isChainedAudio = false; // allow hint after last m4a
            await _playLetterSound('clConsonant.m4a');
          });
        },
      );
      _acquireSoundLock(timeoutSeconds: 6);
      ...
      // Page 4 — TTS → trConsonant slow (mid-chain) → flConsonant slow (last).
      // Hint fires after flConsonant completes via onPlayerComplete.
      _registerTtsHandlers(
        onComplete: () async {
          await Future.delayed(const Duration(milliseconds: 300));
          _isChainedAudio = true; // suppress hint after first m4a
          await _playLetterSound('trConsonant.m4a', slowPlayback: true);
          _audioPlayer.onPlayerComplete.first.then((_) async {
            await Future.delayed(const Duration(milliseconds: 400));
            _isChainedAudio = false; // allow hint after last m4a
            await _playLetterSound('flConsonant.m4a', slowPlayback: true);
          });
        },
      );
      _acquireSoundLock(timeoutSeconds: 6);
  ...
  Future<void> _playSpeakerForCurrentPage() async {
    ...
    if (_currentPage == 0) {
      ...
      // Replay chained m4a pair; hint fires after clConsonant completes.
      _isChainedAudio = true; // suppress hint after first m4a
      await _playLetterSound('blConsonant.m4a');
      _audioPlayer.onPlayerComplete.first.then((_) async {
        await Future.delayed(const Duration(milliseconds: 400));
      ...
      // Replay chained slow m4a pair; hint fires after flConsonant completes.
      _isChainedAudio = true; // suppress hint after first m4a
      await _playLetterSound('trConsonant.m4a', slowPlayback: true);
      _audioPlayer.onPlayerComplete.first.then((_) async {
        await Future.delayed(const Duration(milliseconds: 400));
  ...
  void _navigateToExercise() async {
    ...
    Navigator.of(context)
        .push(
          MaterialPageRoute(
            builder: (context) => const CourseOneLec5Exercise(),
          ),
        )
        .then((result) async {
          await _flutterTts.stop();
      ...
      child: Column(
        ...
        children: [
          Expanded(
            child: Center(
              child: Image.asset(
                'assets/images/stage1Lesson5.png',
                fit: BoxFit.contain,
                errorBuilder:
                    (_, __, ___) =>
  ...
  Widget _buildBlendSounds() {
    const blocks = [
      {'image': 'blBlock.png', 'sound': 'blConsonant.m4a'},
      {'image': 'clBlock.png', 'sound': 'clConsonant.m4a'},
      {'image': 'trBlock.png', 'sound': 'trConsonant.m4a'},
    ];
    return Padding(
