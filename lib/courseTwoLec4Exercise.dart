...
class CourseTwoLec4Exercise extends StatefulWidget {
  const CourseTwoLec4Exercise({super.key});

  @override
  State<CourseTwoLec4Exercise> createState() => _CourseTwoLec4ExerciseState();
}

class _CourseTwoLec4ExerciseState extends State<CourseTwoLec4Exercise>
  ...
  void _selectRandomQuestions() {
    ...
    currentQuizQuestions =
        selected.map((q) {
          ...
          final correctLabel =
              options[q['correctAnswer'] as int]['label'] as String;
          options.shuffle(_random);
          final newCorrectIndex = options.indexWhere(
  ...
  Future<void> _initializeTts() async {
    try {
      ...
      // After the instruction finishes, auto-play the word.
  ...
  /// Called when the instruction TTS finishes — hands off to the word.
  ...
  Future<void> _playCurrentSound() async {
    ...
    try {
      ...
      // Restore the instruction→word chain for this question cycle.
      _flutterTts.setCompletionHandler(_onInstructionComplete);
      await _flutterTts.speak("Which word rhymes?");
    } catch (e) {
  ...
  /// Speaks the current question's word via TTS.
  ...
  Future<void> _playWordSound() async {
    ...
    final word = currentQuizQuestions[currentQuestion - 1]['audio'] as String;
    setState(() => _isSoundPlaying = true);
    try {
      await _flutterTts.stop();
      // Clear the instruction handler so the word's completion only
      // releases the lock — no loop.
      _flutterTts.setCompletionHandler(() {
        if (!_disposed && mounted) {
          setState(() => _isSoundPlaying = false);
          // Restore the instruction handler for the next question cycle.
          _flutterTts.setCompletionHandler(_onInstructionComplete);
        }
      });
  ...
  /// Speaks the tapped option's label word via TTS.
  Future<void> _playOptionSound(String word) async {
    if (_isSoundPlaying) return;
    setState(() => _isSoundPlaying = true);
    try {
      await _flutterTts.stop();
      _flutterTts.setCompletionHandler(() {
        if (!_disposed && mounted) {
          setState(() => _isSoundPlaying = false);
          // Restore instruction handler for the next question cycle.
          _flutterTts.setCompletionHandler(_onInstructionComplete);
        }
      });
      await _flutterTts.speak(word);
    } catch (e) {
      debugPrint('Option sound error: $e');
      setState(() => _isSoundPlaying = false);
      _flutterTts.setCompletionHandler(_onInstructionComplete);
    }
  }
  ...
  void selectAnswer(int index) {
    ...
    final String selectedWord = options[index]['label'] as String;

    _playOptionSound(selectedWord).then((_) {
      _waitForSoundThen(() {
        if (_disposed || !mounted) return;
  ...
  void _handleIncorrectFlow(
    ...
    final options = questionData['options'] as List<Map<String, dynamic>>;
    final String correctWord = options[correctIndex]['label'] as String;
    ...
    _playFeedbackSound(false).then((_) {
      ...
      _waitForSoundThen(() async {
        ...
        await _playOptionSound(correctWord);
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      isCorrect
                          ? 'Great job identifying the rhyme!'
  ...
  Future<void> _saveProgress() async {
    ...
    if (!wasCompleted &&
        _progressManager.isLessonFullyCompleted('course_two', 3)) {
