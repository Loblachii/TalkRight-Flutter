...
import 'dart:async';
import 'courseOneLec3Exercise.dart';
import 'progress_manager.dart';
import 'settings_manager.dart';
import 'notification_helper.dart';

class CourseOneLec3 extends StatefulWidget {
  final bool showModal;

  const CourseOneLec3({super.key, this.showModal = false});

  @override
  State<CourseOneLec3> createState() => _CourseOneLec3State();
}

class _CourseOneLec3State extends State<CourseOneLec3>
  ...
  static const List<String> _ttsPhrases = [
    "Some sounds pop. Some sounds flow.",
    "Clear popping consonant sounds",
    "Extended flowing sounds",
    "Slow, emphasized pronunciation",
  ];
  ...
  void initState() {
    ...
    // Listen for audio completion to release sound lock and trigger hint on
    // last page.
  ...
  void _navigateToExercise() async {
    ...
    Navigator.of(context)
        .push(
          MaterialPageRoute(
            builder: (context) => const CourseOneLec3Exercise(),
          ),
        )
        .then((result) async {
          _releaseSoundLock();
      ...
      child: Column(
        ...
        children: [
          Expanded(
            child: Center(
              child: Image.asset(
                'assets/images/stage1Lesson3.png',
                fit: BoxFit.contain,
                errorBuilder:
                    (_, __, ___) =>
  ...
  // ─── Helper: single animated block at an explicit size ───
  ...
  // ─── Helper: 2 buttons top row, 1 centered below — fully responsive ───
  Widget _buildThreeButtonGrid(
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final blockSize = (w * 0.38).clamp(60.0, 160.0).toDouble();
  ...
  // ─── Helper: centered row of blocks — fully responsive ───
  Widget _buildSoundButtonRow(
    List<Map<String, String>> blocks, {
    bool slowPlayback = false,
  }) {
    return LayoutBuilder(
      builder: (context, constraints) {
        final w = constraints.maxWidth.isFinite ? constraints.maxWidth : 300.0;
        final blockSize =
            (w / (blocks.length + 1)).clamp(60.0, 160.0).toDouble();
        return FittedBox(
          fit: BoxFit.scaleDown,
          child: Row(
            mainAxisAlignment: MainAxisAlignment.center,
  ...
  // ─── Page 2: Pop Sounds — 3 buttons: B, T, P ───
  Widget _buildPopSounds() {
    const blocks = [
      {'image': 'bBlock.png', 'sound': 'bSound.m4a'},
      {'image': 'tBlock.png', 'sound': 'tSound.m4a'},
      {'image': 'pBlock.png', 'sound': 'pSound.m4a'},
    ];
    return Padding(
      padding: const EdgeInsets.all(32),
      child: Column(
  ...
  // ─── Page 3: Soft / Continuous Sound — 3 buttons: S, F, M ───
  Widget _buildSoftSounds() {
    const blocks = [
      {'image': 'sBlock.png', 'sound': 'sSound.m4a'},
      {'image': 'fBlock.png', 'sound': 'fSound.m4a'},
  ...
  // ─── Page 4: Sound Focus — 2 buttons: B, F — slow playback ───
  Widget _buildSoundFocus() {
    const blocks = [
      {'image': 'bBlock.png', 'sound': 'bSound.m4a'},
      {'image': 'fBlock.png', 'sound': 'fSound.m4a'},
    ];
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Expanded(
            child: Center(
              child: _buildSoundButtonRow(blocks, slowPlayback: true),
            ),
          ),
          const SizedBox(height: 16),
          const Text(
