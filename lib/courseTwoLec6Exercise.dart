...
class CourseTwoLec6Exercise extends StatefulWidget {
  const CourseTwoLec6Exercise({super.key});

  @override
  State<CourseTwoLec6Exercise> createState() => _CourseTwoLec6ExerciseState();
}

class _CourseTwoLec6ExerciseState extends State<CourseTwoLec6Exercise>
  ...
  // ── 10 word-building questions (5 shown per session, randomized) ──────────
  final List<Map<String, dynamic>> allQuizData = [
    // 1️⃣  Audio: helpful  Choices: helpful | helping | helped  → correct @ 0
    {
      'questionWord': 'helpful',
      'answers': [
        {'label': 'helpful'},
        {'label': 'helping'},
        {'label': 'helped'},
    ...
    // 2️⃣  Audio: helping  Choices: helped | helping | helpful  → correct @ 1
    {
      'questionWord': 'helping',
      'answers': [
        {'label': 'helped'},
        {'label': 'helping'},
        {'label': 'helpful'},
    ...
    // 3️⃣  Audio: helped   Choices: helping | helpful | helped  → correct @ 2
    {
      'questionWord': 'helped',
      'answers': [
        {'label': 'helping'},
    ...
    // 4️⃣  Audio: playing  Choices: played | playful | playing  → correct @ 2
    {
      'questionWord': 'playing',
      'answers': [
        {'label': 'played'},
        {'label': 'playful'},
        {'label': 'playing'},
    ...
    // 5️⃣  Audio: played   Choices: playing | playful | played  → correct @ 2
    {
      'questionWord': 'played',
      'answers': [
        {'label': 'playing'},
    ...
    // 6️⃣  Audio: hopeful  Choices: hoping | hopeful | hoped    → correct @ 1
    {
      'questionWord': 'hopeful',
      'answers': [
        {'label': 'hoping'},
        {'label': 'hopeful'},
        {'label': 'hoped'},
    ...
    // 7️⃣  Audio: hoped    Choices: hoping | hoped | hopeful    → correct @ 1
    {
      'questionWord': 'hoped',
      'answers': [
        {'label': 'hoping'},
    ...
    // 8️⃣  Audio: running  Choices: running | runner | runned   → correct @ 0
    {
      'questionWord': 'running',
      'answers': [
        {'label': 'running'},
        {'label': 'runner'},
        {'label': 'runned'},
    ...
    // 9️⃣  Audio: jumped   Choices: jumping | jumped | jumper   → correct @ 1
    {
      'questionWord': 'jumped',
      'answers': [
        {'label': 'jumping'},
    ...
    // 🔟  Audio: careful   Choices: caring | careful | cared    → correct @ 1
    {
      'questionWord': 'careful',
      'answers': [
        {'label': 'caring'},
        {'label': 'careful'},
        {'label': 'cared'},
  ...
  /// Speaks the fixed instruction, then auto-speaks the question word
  ...
  Future<void> _playCurrentSound() async {
    ...
    try {
      ...
      // One-shot: fires after the instruction, then immediately restores
      // the default (lock-release-only) handler so the word doesn't loop.
      _flutterTts.setCompletionHandler(() {
        if (_disposed || !mounted) return;
        // Restore default handler before speaking the word
        _flutterTts.setCompletionHandler(() {
          if (!_disposed && mounted) setState(() => _isSoundPlaying = false);
        });
        setState(() => _isSoundPlaying = false);
      ...
      // Restore default handler on error too
      _flutterTts.setCompletionHandler(() {
        if (!_disposed && mounted) setState(() => _isSoundPlaying = false);
      });
      setState(() => _isSoundPlaying = false);
  ...
  /// Speaks only the question word for the current question via TTS.
  ...
  Future<void> _playQuestionAudio() async {
    ...
    final word =
        currentQuizQuestions[currentQuestion - 1]['questionWord'] as String;
    await _speakWord(word);
  }

  /// Speaks a word via TTS and acquires the sound lock.
  /// The default completion handler (set in _initializeTts) releases the lock.
  ...
  void selectAnswer(int index) {
    ...
    final word = (answers[index] as Map)['label'] as String;
    _flutterTts.stop();
    _audioPlayer.stop();

    _speakWord(word).then((_) {
      _waitForSoundThen(() {
        if (_disposed || !mounted) return;
    ...
    _playFeedbackSound(true).then((_) {
      ...
      _waitForSoundThen(() {
        ...
        _showListenRepeatOverlay(
          questionData['questionWord'] as String,
  ...
  void _handleIncorrectFlow(
    ...
    final String correctWord =
        (answers[correctIndex] as Map)['label'] as String;
  ...
  Future<void> _saveProgress() async {
    ...
    if (!wasCompleted &&
        _progressManager.isLessonFullyCompleted('course_two', 5)) {
      NotificationHelper.onLessonComplete('course_two', 5, 'Morphology Basics');
    }
  }
                          ...
                          child: Column(
                            ...
                            children: List.generate(answers.length, (index) {
                              ...
                              final bool dimmed =
                                  ...
                                  !_answerRevealed &&
                                  selectedAnswerIndex != index;
                              final String label = answers[index]['label']!;
