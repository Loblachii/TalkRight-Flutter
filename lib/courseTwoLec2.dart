...
import 'dart:async';
import 'courseTwoLec2Exercise.dart';
import 'progress_manager.dart';
import 'settings_manager.dart';
import 'notification_helper.dart';

class CourseTwoLec2 extends StatefulWidget {
  final bool showModal;

  const CourseTwoLec2({super.key, this.showModal = false});

  @override
  State<CourseTwoLec2> createState() => _CourseTwoLec2State();
}

class _CourseTwoLec2State extends State<CourseTwoLec2>
  ...
  static const List<String> _ttsPhrases = [
    "Two vowels can make one sound.",
    "Listen to the vowel sound.",
    "Listen to the vowel in the word",
    "Listen carefully.",
  ];
  ...
  /// Releases the TTS portion of the lock without clearing _activeSoundFile.
  /// Used when TTS is chained into an m4a — the m4a will call _releaseSoundLock
  /// via onPlayerComplete when it fully finishes.
  void _releaseTtsLockOnly() {
    _soundLockTimer?.cancel();
    _soundLockTimer = null;
    // Do NOT clear _activeSoundFile or _activeTtsWord here —
    // the m4a chain needs them to stay set so the highlight stays orange.
  }
  ...
  /// Like _registerTtsHandlers but uses _releaseTtsLockOnly on completion
  /// so the highlight stays alive while the chained m4a plays.
  void _registerTtsHandlersChained({required VoidCallback onComplete}) {
    _flutterTts.setCompletionHandler(() {
      _releaseTtsLockOnly();
      onComplete();
    });
  ...
  // ─── Page-load: TTS phrase → then auto-play m4a(s) on pages 3 & 4 ────────
  Future<void> _playOnPageLoad() async {
    ...
    if (_currentPage == 0) {
      ...
      // Page 3 — TTS → eaVowelTeam (mid-chain) → ieVowelTeam (last).
      // Hint fires after ieVowelTeam completes via onPlayerComplete.
      _registerTtsHandlers(
        onComplete: () async {
          await Future.delayed(const Duration(milliseconds: 300));
          _isChainedAudio = true; // suppress hint after first m4a
          await _playLetterSound('eaVowelTeam.m4a');
          _audioPlayer.onPlayerComplete.first.then((_) async {
            await Future.delayed(const Duration(milliseconds: 400));
            _isChainedAudio = false; // allow hint after last m4a
            await _playLetterSound('ieVowelTeam.m4a');
          });
        },
      );
      _acquireSoundLock(timeoutSeconds: 6);
      ...
      // Page 4 — TTS → iSound (mid-chain) → eaVowelTeam (last).
      // Hint fires after eaVowelTeam completes via onPlayerComplete.
      _registerTtsHandlers(
        onComplete: () async {
          await Future.delayed(const Duration(milliseconds: 300));
          _isChainedAudio = true; // suppress hint after first m4a
          await _playLetterSound('iSound.m4a');
          _audioPlayer.onPlayerComplete.first.then((_) async {
            await Future.delayed(const Duration(milliseconds: 400));
            _isChainedAudio = false; // allow hint after last m4a
            await _playLetterSound('eaVowelTeam.m4a');
  ...
  Future<void> _playSpeakerForCurrentPage() async {
    ...
    if (_currentPage == 0) {
      ...
      // Replay chained m4a pair; hint fires after ieVowelTeam completes.
      ...
      // Replay chained m4a pair; hint fires after eaVowelTeam completes.
      _isChainedAudio = true;
      await _playLetterSound('iSound.m4a');
      _audioPlayer.onPlayerComplete.first.then((_) async {
        await Future.delayed(const Duration(milliseconds: 400));
  ...
  void _navigateToExercise2() async {
    ...
    Navigator.of(context)
        .push(
          MaterialPageRoute(
            builder: (context) => const CourseTwoLec2Exercise(),
          ),
        )
        .then((result) async {
          await _flutterTts.stop();
      ...
      child: Column(
        ...
        children: [
          Expanded(
            child: Center(
              child: Image.asset(
                'assets/images/stage2Lesson2.png',
                fit: BoxFit.contain,
                errorBuilder:
                    (_, __, ___) =>
                  ...
                  child: Column(
                    ...
                    children: [
                        ...
                        children: [
                          SizedBox(
                            ...
                            child: _AnimatedBlock(
                              imageName: 'eaVowelTeam.png',
                              soundName: 'eaVowelTeam.m4a',
                              isSoundPlaying: _isSoundPlaying,
                              isActiveBlock:
                                  _activeSoundFile == 'eaVowelTeam.m4a',
                              onTap: () => _playLetterSound('eaVowelTeam.m4a'),
                            ),
                          ),
                          SizedBox(width: gap),
                          SizedBox(
                            ...
                            height: blockSize,
                            child: _AnimatedBlock(
                              imageName: 'ieVowelTeam.png',
                              soundName: 'ieVowelTeam.m4a',
                              isSoundPlaying: _isSoundPlaying,
                              isActiveBlock:
                                  _activeSoundFile == 'ieVowelTeam.m4a',
                              onTap: () => _playLetterSound('ieVowelTeam.m4a'),
                            ),
                          ),
                        ],
                      ),
                      ...
                      SizedBox(
                        ...
                        height: blockSize,
                        child: _AnimatedBlock(
                          imageName: 'oaVowelTeam.png',
                          soundName: 'oaVowelTeam.m4a',
                          isSoundPlaying: _isSoundPlaying,
                          isActiveBlock: _activeSoundFile == 'oaVowelTeam.m4a',
                          onTap: () => _playLetterSound('oaVowelTeam.m4a'),
                        ),
                      ),
                    ],
                  ),
  ...
  Widget _buildEmojiWordButton({
    ...
    return _AnimatedEmojiCard(
      ...
      onTap: () async {
        ...
        // Set _activeSoundFile NOW so the card turns orange immediately
        // and stays orange through both the TTS and the m4a.
        if (mounted) {
          setState(() {
            _isSoundPlaying = true;
            _activeTtsWord = word;
            _activeSoundFile = soundFile;
          });
        }

        // Use the chained variant — does NOT clear _activeSoundFile when TTS
        // finishes, so the highlight stays orange while the m4a plays.
        _registerTtsHandlersChained(
          onComplete: () async {
            await Future.delayed(const Duration(milliseconds: 200));
            await _playLetterSound(soundFile);
          },
        );

        _acquireSoundLock(timeoutSeconds: 10);
