...
import 'dart:async';
import 'courseTwoLec5Exercise.dart';
import 'progress_manager.dart';
import 'settings_manager.dart';
import 'notification_helper.dart';

class CourseTwoLec5 extends StatefulWidget {
  final bool showModal;

  const CourseTwoLec5({super.key, this.showModal = false});

  @override
  State<CourseTwoLec5> createState() => _CourseTwoLec5State();
}

class _CourseTwoLec5State extends State<CourseTwoLec5>
  ...
  String? _activeWord; // tracks which word button is currently speaking
  ...
  /// Normal-speed TTS used on pages 2 & 3 word button taps.
  ...
  /// Reduced-speed TTS used on page 4 word button taps — slower for clarity.
  ...
  void _navigateToExercise() async {
    ...
    Navigator.of(context)
        .push(
          MaterialPageRoute(
            builder: (context) => const CourseTwoLec5Exercise(),
          ),
        )
        .then((result) async {
          await _flutterTts.stop();
      ...
      child: Column(
        ...
        children: [
          Expanded(
            child: Center(
              child: Image.asset(
                'assets/images/stage2Lesson5.png',
                fit: BoxFit.contain,
                errorBuilder:
                    (_, __, ___) =>
  ...
  // ─── Helper: orange word button with tap animation + active indicator ────
  ...
  // ─── Page 2: Irregular Word Sounds — said, one, two (normal speed) ────────
  Widget _buildWordFamilySounds() {
    return LayoutBuilder(
      builder: (context, constraints) {
        final double h = constraints.maxHeight;
        final double captionSize = (h * 0.05).clamp(12.0, 18.0);
        final double spacing = (h * 0.04).clamp(10.0, 22.0);
                ...
                child: Center(
                  child: Column(
                    ...
                    children: [
                      _buildWordButton(
                        label: 'said',
                        onTap: () => _speakWord('said'),
                      ),
                      SizedBox(height: spacing),
                      _buildWordButton(
                        label: 'one',
                        onTap: () => _speakWord('one'),
                      ),
                      SizedBox(height: spacing),
                      _buildWordButton(
                        label: 'two',
                        onTap: () => _speakWord('two'),
                      ),
                    ],
                  ),
                ),
  ...
  // ─── Page 3: Listen & Compare — said | sad (normal speed) ────────────────
  ...
  // ─── Page 4: Reinforcement — said | sad (SLOW speed) ─────────────────────
                ...
                child: Center(
                  child: Row(
                    ...
                    children: [
                      _buildWordButton(
                        label: 'said',
                        onTap: () => _speakWordSlow('said'),
                        horizontalPadding: 28,
                      ),
                      SizedBox(width: spacing * 2),
                      _buildWordButton(
                        label: 'sad',
                        onTap: () => _speakWordSlow('sad'),
                        horizontalPadding: 28,
                      ),
                    ],
                  ),
