...
import 'dart:async';
import 'courseTwoLec4Exercise.dart';
import 'progress_manager.dart';
import 'settings_manager.dart';
import 'notification_helper.dart';

class CourseTwoLec4 extends StatefulWidget {
  final bool showModal;

  const CourseTwoLec4({super.key, this.showModal = false});

  @override
  State<CourseTwoLec4> createState() => _CourseTwoLec4State();
}

class _CourseTwoLec4State extends State<CourseTwoLec4>
  ...
  String? _activeWord; // tracks which emoji card is currently speaking
  ...
  void _releaseSoundLock() {
    ...
    // Always restore default rate so slow-playback from screen 4
  ...
  /// Normal-speed TTS used on pages 2 & 3 emoji taps.
  Future<void> _speakWord(String word) async {
    if (_isSoundPlaying) return;
    _acquireSoundLock();
    if (mounted) setState(() => _activeWord = word);
  ...
  /// Reduced-speed TTS used on page 4 emoji taps — slower = clearer ending.
  Future<void> _speakWordSlow(String word) async {
    if (_isSoundPlaying) return;
    _acquireSoundLock();
    if (mounted) setState(() => _activeWord = word);
    _registerTtsHandlers();
  ...
  // ─── Page-load: TTS phrase auto-plays on every page ──────────────────────
  ...
  // ─── Speaker button: replay TTS phrase for the current page ──────────────
  Future<void> _playSpeakerForCurrentPage() async {
    ...
    // on the auto page-load TTS, matching Lec3 behaviour.
    await _flutterTts.setSpeechRate(0.5);
    await _flutterTts.speak(_ttsPhrases[_currentPage]);
  }
  ...
  void _navigateToExercise() async {
    ...
    Navigator.of(context)
        .push(
          MaterialPageRoute(
            builder: (context) => const CourseTwoLec4Exercise(),
          ),
        )
        .then((result) async {
          await _flutterTts.stop();
          ...
          body: SafeArea(
            child: Column(
              children: [
                ...
                // ─── 2. BODY: Title, white container, speaker ───
                Expanded(
                  child: FadeTransition(
                ...
                // ─── 3. BOTTOM: Page dots + Exercise button ───
                Padding(
      ...
      child: Column(
        ...
        children: [
          Expanded(
            child: Center(
              child: Image.asset(
                'assets/images/stage2Lesson4.png',
                fit: BoxFit.contain,
                errorBuilder:
                    (_, __, ___) =>
  ...
  // ─── Page 2: Word Family Sounds — Cat 🐱, Bat 🦇, Hat 🎩 ─────────────────
  Widget _buildWordFamilySounds() {
    return LayoutBuilder(
      builder: (context, constraints) {
        final double h = constraints.maxHeight;
        final double w = constraints.maxWidth;
        final double blockSize = ((h - 76) / 2.37)
            .clamp(0.0, 100.0)
            .clamp(0.0, w * 0.42);
        final double gap = (blockSize * 0.25).clamp(8.0, 28.0);
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          _AnimatedEmojiCard(
                            ...
                            boxSize: blockSize,
                            isSoundPlaying: _isSoundPlaying,
                            isActive: _activeWord == 'Bat',
                            onTap: () => _speakWord('Bat'),
                          ),
                        ],
                      ),
                      SizedBox(height: gap),
                      _AnimatedEmojiCard(
                        ...
                        boxSize: blockSize,
                        isSoundPlaying: _isSoundPlaying,
                        isActive: _activeWord == 'Hat',
                        onTap: () => _speakWord('Hat'),
                      ),
                    ],
                  ),
                ),
  ...
  // ─── Page 3: Sound Pattern — Sat 🪑, Rat 🐭, Pat 👋 ──────────────────────
  Widget _buildSoundPattern() {
    return LayoutBuilder(
      builder: (context, constraints) {
        final double h = constraints.maxHeight;
        final double w = constraints.maxWidth;
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          _AnimatedEmojiCard(
                            ...
                            boxSize: blockSize,
                            isSoundPlaying: _isSoundPlaying,
                            isActive: _activeWord == 'Sat',
                            onTap: () => _speakWord('Sat'),
                          ),
                          SizedBox(width: gap),
                          _AnimatedEmojiCard(
                            emoji: '🐭',
                            ...
                            boxSize: blockSize,
                            isSoundPlaying: _isSoundPlaying,
                            isActive: _activeWord == 'Rat',
                            onTap: () => _speakWord('Rat'),
                          ),
                        ],
                      ),
                      SizedBox(height: gap),
                      _AnimatedEmojiCard(
                        ...
                        boxSize: blockSize,
                        isSoundPlaying: _isSoundPlaying,
                        isActive: _activeWord == 'Pat',
                        onTap: () => _speakWord('Pat'),
                      ),
                    ],
                  ),
                ),
  ...
  // ─── Page 4: Compare Rhymes — Hat 🎩, Hot 🔥 (slow speed) ───────────────
  Widget _buildCompareRhymes() {
    return Padding(
      padding: const EdgeInsets.all(32),
      child: LayoutBuilder(
        builder: (context, constraints) {
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      _AnimatedEmojiCard(
                        ...
                        boxSize: blockSize,
                        isSoundPlaying: _isSoundPlaying,
                        isActive: _activeWord == 'Hot',
                        onTap: () => _speakWordSlow('Hot'),
                      ),
                    ],
                  ),
                ),
...
// ─── Animated emoji card (tap-bounce + dimming on inactive) ──────────────────
...
class _AnimatedEmojiCardState extends State<_AnimatedEmojiCard>
  ...
  static const Color _cardColor = Color.fromARGB(255, 176, 184, 176);
  ...
  @override
  Widget build(BuildContext context) {
    final double emojiSize = (widget.boxSize * 0.48).clamp(24.0, 56.0);
    final double fontSize = (widget.boxSize * 0.20).clamp(11.0, 22.0);
    final double radius = (widget.boxSize * 0.18).clamp(10.0, 22.0);

    // Dim inactive cards while any sound is playing; active card stays full.
    final bool isDimmed = widget.isSoundPlaying && !widget.isActive;
        ...
        child: ScaleTransition(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: widget.boxSize * 0.06),
              Text(
                widget.label,
                style: TextStyle(
                  fontFamily: 'Fredoka',
