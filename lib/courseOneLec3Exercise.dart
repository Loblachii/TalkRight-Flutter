...
class CourseOneLec3Exercise extends StatefulWidget {
  const CourseOneLec3Exercise({super.key});

  @override
  State<CourseOneLec3Exercise> createState() => _CourseOneLec3ExerciseState();
}

class _CourseOneLec3ExerciseState extends State<CourseOneLec3Exercise>
  ...
  final List<Map<String, dynamic>> allQuizData = [
    {
      'instruction': 'Tap the popping sound.',
      'sound': 'bSound.m4a',
      'answers': ['B', 'M', 'S'],
      'correctAnswer': 0,
    },
    ...
    {
      'instruction': 'Tap the soft sound.',
      'sound': 'mSound.m4a',
      'answers': ['B', 'M', 'T'],
      'correctAnswer': 1,
    },
  ...
  Future<void> _playCurrentSound() async {
    ...
    // ← Restore the handler that was cleared during answer selection
    _flutterTts.setCompletionHandler(() {
      if (!_disposed && mounted) {
        setState(() => _isSoundPlaying = false);
        _playLetterSound();
    ...
    try {
      ...
      final instruction =
          currentQuizQuestions[currentQuestion - 1]['instruction'] as String;
  ...
  //   • Correct feedback sound + snackbar
  ...
  void selectAnswer(int index) {
    ...
    final String label = questionData['answers'][index] as String;
    ...
    // Clear the TTS completion handler so it doesn't fire mid-flow
    _flutterTts.setCompletionHandler(() {});
    _flutterTts.stop();

    _playAsset(_soundFileForLabel(label)).then((_) {
      _waitForSoundThen(() {
        if (_disposed || !mounted) return;

        // Step 2: sound done — now reveal result
        setState(() {
  ...
  Future<void> _saveProgress() async {
    ...
    if (!wasCompleted &&
        _progressManager.isLessonFullyCompleted('course_one', 2)) {
      NotificationHelper.onLessonComplete('course_one', 2, 'Consonant Sounds');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final String instruction =
        questionData != null
            ? questionData['instruction'] as String
