...
class CourseTwoLec5Exercise extends StatefulWidget {
  const CourseTwoLec5Exercise({super.key});

  @override
  State<CourseTwoLec5Exercise> createState() => _CourseTwoLec5ExerciseState();
}

class _CourseTwoLec5ExerciseState extends State<CourseTwoLec5Exercise>
  ...
  // ── 10 irregular-word questions ──────────────────────────────────────────
  // correctAnswer distribution: 0×4, 1×3, 2×3
  final List<Map<String, dynamic>> allQuizData = [
    // ── correctAnswer: 0 (×4) ────────────────────────────────────────────

    // 4️⃣  Audio: was   Choices: was | has | us   → correct @ 0
    {
      'questionWord': 'was',
      'answers': [
        {'label': 'was'},
    ...
    // 5️⃣  Audio: come  Choices: come | comb | calm  → correct @ 0
    {
      'questionWord': 'come',
      'answers': [
        {'label': 'come'},
    ...
    // 7️⃣  Audio: have  Choices: have | hive | gave  → correct @ 0
    {
      'questionWord': 'have',
      'answers': [
        {'label': 'have'},
    ...
    // 8️⃣  Audio: give  Choices: give | live | five  → correct @ 0
    {
      'questionWord': 'give',
      'answers': [
        {'label': 'give'},
    ...
    // 1️⃣  Audio: said  Choices: sad | said | sail  → correct @ 1
    {
      'questionWord': 'said',
      'answers': [
        {'label': 'sad'},
    ...
    // 2️⃣  Audio: one   Choices: on | one | won    → correct @ 1
    {
      'questionWord': 'one',
      'answers': [
        {'label': 'on'},
    ...
    // 6️⃣  Audio: does  Choices: dose | does | goes → correct @ 1
    {
      'questionWord': 'does',
      'answers': [
        {'label': 'dose'},
    ...
    // 3️⃣  Audio: two   Choices: too | two | toe   → correct @ 2
    {
      'questionWord': 'two',
      'answers': [
        {'label': 'clue'},
    ...
    // 9️⃣  Audio: some  Choices: same | sun | some  → correct @ 2
    {
      'questionWord': 'some',
      'answers': [
        {'label': 'same'},
    ...
    // 🔟  Audio: were  Choices: where | wear | were → correct @ 2
    {
      'questionWord': 'were',
      'answers': [
        {'label': 'here'},
  ...
  Future<void> _saveProgress() async {
    ...
    if (!wasCompleted &&
        _progressManager.isLessonFullyCompleted('course_two', 4)) {
      NotificationHelper.onLessonComplete('course_two', 4, 'Irregular Words');
    }
  }
...
/// Wraps any widget with a bounce animation (shrink → overshoot → settle).
/// Pass a no-op `() {}` as [onTap] when the button should be visually
/// disabled — pair with [AnimatedOpacity] for the disabled look.
