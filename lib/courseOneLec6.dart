...
import 'dart:async';
import 'courseOneLec6Exercise.dart';
import 'progress_manager.dart';
import 'settings_manager.dart';
import 'notification_helper.dart';

class CourseOneLec6 extends StatefulWidget {
  final bool showModal;

  const CourseOneLec6({super.key, this.showModal = false});

  @override
  State<CourseOneLec6> createState() => _CourseOneLec6State();
}

class _CourseOneLec6State extends State<CourseOneLec6>
  ...
  // ─── Page-load: TTS phrase → then auto-play m4a(s) on pages 2 & 3 ────────
  Future<void> _playOnPageLoad() async {
    ...
    if (_currentPage == 0) {
      ...
      // Page 3 — TTS → ndConsonantEnd (mid-chain) → stConsonantEnd (last).
      // Hint fires after stConsonantEnd completes via onPlayerComplete.
      _registerTtsHandlers(
        onComplete: () async {
          await Future.delayed(const Duration(milliseconds: 300));
          _isChainedAudio = true; // suppress hint after first m4a
          await _playLetterSound('ndConsonantEnd.m4a');
          _audioPlayer.onPlayerComplete.first.then((_) async {
            await Future.delayed(const Duration(milliseconds: 400));
            _isChainedAudio = false; // allow hint after last m4a
            await _playLetterSound('stConsonantEnd.m4a');
          });
        },
      );
      _acquireSoundLock(timeoutSeconds: 6);
      ...
      // Page 4 — TTS → mpConsonantEnd (mid-chain) → ftConsonantEnd (last).
      // Hint fires after ftConsonantEnd completes via onPlayerComplete.
      _registerTtsHandlers(
        onComplete: () async {
          await Future.delayed(const Duration(milliseconds: 300));
          _isChainedAudio = true; // suppress hint after first m4a
          await _playLetterSound('mpConsonantEnd.m4a');
          _audioPlayer.onPlayerComplete.first.then((_) async {
            await Future.delayed(const Duration(milliseconds: 400));
            _isChainedAudio = false; // allow hint after last m4a
            await _playLetterSound('ftConsonantEnd.m4a');
          });
        },
      );
      _acquireSoundLock(timeoutSeconds: 6);
  ...
  Future<void> _playSpeakerForCurrentPage() async {
    ...
    if (_currentPage == 0) {
      ...
      // Replay chained m4a pair; hint fires after stConsonantEnd completes.
      _isChainedAudio = true; // suppress hint after first m4a
      await _playLetterSound('ndConsonantEnd.m4a');
      _audioPlayer.onPlayerComplete.first.then((_) async {
        await Future.delayed(const Duration(milliseconds: 400));
      ...
      // Replay chained m4a pair; hint fires after ftConsonantEnd completes.
      _isChainedAudio = true; // suppress hint after first m4a
      await _playLetterSound('mpConsonantEnd.m4a');
      _audioPlayer.onPlayerComplete.first.then((_) async {
        await Future.delayed(const Duration(milliseconds: 400));
  ...
  void _navigateToExercise() async {
    ...
    Navigator.of(context)
        .push(
          MaterialPageRoute(
            builder: (context) => const CourseOneLec6Exercise(),
          ),
        )
        .then((result) async {
          await _flutterTts.stop();
                                        ...
                                        GestureDetector(
                                          ...
                                          child: PageView(
                                            ...
                                            children: [
                                              ...
                                              _buildEndingBlends(),
                                              _buildWordsWithEndingBlends(),
                                              _buildSoundAwareness(),
                                            ],
                                          ),
                                        ),
      ...
      child: Column(
        ...
        children: [
          Expanded(
            child: Center(
              child: Image.asset(
                'assets/images/stage1Lesson6.png',
                fit: BoxFit.contain,
                errorBuilder:
                    (_, __, ___) =>
  ...
  Widget _buildSoundBlock(Map<String, String> block, {required double size}) {
    return SizedBox(
      width: size,
      height: size,
  ...
  Widget _buildThreeButtonGrid(List<Map<String, String>> blocks) {
    return LayoutBuilder(
      builder: (context, constraints) {
        final double blockSize = (constraints.maxWidth * 0.40).clamp(
          64.0,
          160.0,
        );
        final double gap = (constraints.maxWidth * 0.08).clamp(10.0, 28.0);
        ...
        return Column(
          ...
          children: [
            Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                _buildSoundBlock(blocks[0], size: blockSize),
                SizedBox(width: gap),
                _buildSoundBlock(blocks[1], size: blockSize),
              ],
            ),
            SizedBox(height: gap),
            _buildSoundBlock(blocks[2], size: blockSize),
          ],
        );
      },
    );
  ...
  Widget _buildEndingBlends() {
    const blocks = [
      {'image': 'ndBlock.png', 'sound': 'ndConsonantEnd.m4a'},
      {'image': 'stBlock.png', 'sound': 'stConsonantEnd.m4a'},
      {'image': 'mpBlock.png', 'sound': 'mpConsonantEnd.m4a'},
    ];
    return Padding(
      padding: const EdgeInsets.all(32),
      child: Column(
        ...
        crossAxisAlignment: CrossAxisAlignment.center,
        children: [
          Expanded(child: Center(child: _buildThreeButtonGrid(blocks))),
          const SizedBox(height: 16),
          const Text(
            '"Ending sounds"',
            style: TextStyle(
  ...
  Widget _buildWordsWithEndingBlends() {
    return Padding(
...
// ─── Standalone animated block widget (Page 2) ───────────────────────────────
...
// ─── Animated emoji card widget (Pages 3 & 4) ────────────────────────────────
