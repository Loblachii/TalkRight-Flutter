...
import 'courseThreeLec1Exercise.dart';
import 'progress_manager.dart';
import 'settings_manager.dart';
import 'notification_helper.dart';

class CourseThreeLec1 extends StatefulWidget {
  final bool showModal;

  const CourseThreeLec1({super.key, this.showModal = false});

  @override
  State<CourseThreeLec1> createState() => _CourseThreeLec1State();
}

class _CourseThreeLec1State extends State<CourseThreeLec1>
  ...
  // For word-by-word highlighting on page 4
  final List<String> _page4Words = ['The', 'cat', 'eats', 'fish'];
  ...
  Future<void> _playOnPageLoad() async {
    ...
    switch (_currentPage) {
      case 0:
        await _speakAndWait("Words come together to make sentences.");
        break;
      case 1:
        await _speakAndWait("Listen to the whole sentence.");
        break;
      case 2:
        await _speakAndWait("Listen to the longer sentence.");
        break;
      case 3:
        await _speakAndWait("Listen carefully.");
        break;
    }
  ...
  Future<void> _playSpeakerForCurrentPage() async {
    ...
    switch (_currentPage) {
      case 0:
        ...
        break;
      case 1:
        await _speakAndWait("The cat runs.");
        break;
      case 2:
        await _speakAndWait("The cat eats fish.");
        break;
      case 3:
  ...
  /// Speaks page 4 words one by one with highlighting, then repeats the full
  /// sentence at normal speed.
  Future<void> _speakPage4WithHighlight() async {
    // ── Word-by-word chunk ──
    await _flutterTts.setSpeechRate(0.3);

    for (int i = 0; i < _page4Words.length; i++) {
      if (!mounted) break;
      setState(() => _highlightedWordIndex = i);
      await _speakAndWait(_page4Words[i]);
      await Future.delayed(const Duration(milliseconds: 700));
    }

    if (mounted) setState(() => _highlightedWordIndex = -1);

    // ── Full sentence repeat at normal speed with per-word highlighting ──
    await _flutterTts.setSpeechRate(0.5);
    await Future.delayed(const Duration(milliseconds: 400));
    if (!mounted) return;

    const int msPerWord = 450;

    for (int i = 0; i < _page4Words.length; i++) {
      if (!mounted) break;
      setState(() => _highlightedWordIndex = i);
      await _speakAndWait(_page4Words[i]);
      await Future.delayed(const Duration(milliseconds: msPerWord ~/ 4));
    }
  ...
  void _navigateToExercise2() async {
    ...
    Navigator.of(context)
        .push(
          MaterialPageRoute(
            builder: (context) => const CourseThreeLec1Exercise(),
          ),
        )
        .then((result) async {
          _releaseSoundLock();
      ...
      child: Column(
        ...
        children: [
          Expanded(
            child: Center(
              child: Image.asset(
                'assets/images/stage3Lesson1.png',
                fit: BoxFit.contain,
                errorBuilder:
                    (_, __, ___) =>
          ...
          const Text(
            '"Words come together to make sentences."',
            style: TextStyle(
              fontFamily: 'Fredoka',
              fontSize: 20,
              fontWeight: FontWeight.w400,
  ...
  // ─── PAGE 2: Sentence Listening — catRun.png ───
  ...
  // ─── PAGE 3: Expanded Sentence — catEat.png ───
  ...
  // ─── PAGE 4: Sentence Flow — word-by-word highlight ───
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: const Color(0xFF9D5C5D).withOpacity(0.25),
              borderRadius: BorderRadius.circular(12),
            ),
            child: Row(
              mainAxisSize: MainAxisSize.min,
              mainAxisAlignment: MainAxisAlignment.center,
              children: List.generate(_page4Words.length, (index) {
                final bool isHighlighted = _highlightedWordIndex == index;
                return Padding(
