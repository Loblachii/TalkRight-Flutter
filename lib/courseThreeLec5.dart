...
import 'dart:async';
import 'courseThreeLec5Exercise.dart';
import 'progress_manager.dart';
import 'settings_manager.dart';
import 'notification_helper.dart';

class CourseThreeLec5 extends StatefulWidget {
  final bool showModal;

  const CourseThreeLec5({super.key, this.showModal = false});

  @override
  State<CourseThreeLec5> createState() => _CourseThreeLec5State();
}

class _CourseThreeLec5State extends State<CourseThreeLec5>
  ...
  static const List<String> _captionPhrases = [
    'Words can sound smooth together', // Page 1
    'Listen to the difference.', // Page 2
  ...
  /// Reduced-speed TTS — used on page 2 "going to" and page 4 phrase button taps for clarity.
  ...
  // ─── Page-load: speaks caption, acquires lock ─────────────────────────────
  ...
  // ─── Speaker button: replays the same caption spoken on page load ─────────
  ...
  void _navigateToExercise() async {
    ...
    Navigator.of(context)
        .push(
          MaterialPageRoute(
            builder: (context) => const CourseThreeLec5Exercise(),
          ),
        )
        .then((result) async {
          await _flutterTts.stop();
      ...
      child: Column(
        ...
        children: [
          Expanded(
            child: Center(
              child: Image.asset(
                'assets/images/stage3Lesson5.png',
                fit: BoxFit.contain,
                errorBuilder:
                    (_, __, ___) =>
          ...
          const Text(
            '"Words can sound smooth together."',
            style: TextStyle(
              fontFamily: 'Fredoka',
              fontSize: 20,
              fontWeight: FontWeight.w400,
  ...
  // ─── PAGE 2: Slow vs. Natural — "going to" slow, "gonna" normal speed ─────
  Widget _buildSlowVsNatural() {
                ...
                child: Center(
                  child: Column(
                    ...
                    children: [
                      _buildPhraseButton(
                        label: 'going to',
                        onTap: () => _speakPhraseSlow('going to'),
                      ),
                      SizedBox(height: spacing),
                      _buildPhraseButton(
                        label: 'gonna',
                        onTap: () => _speakPhrase('gonna'),
                      ),
                    ],
                  ),
                ),
  ...
  // ─── PAGE 3: Words Together — normal speed ────────────────────────────────
  Widget _buildWordsTogether() {
    return LayoutBuilder(
      builder: (context, constraints) {
        final double h = constraints.maxHeight;
        final double captionSize = (h * 0.05).clamp(12.0, 20.0);
  ...
  // ─── PAGE 4: Natural Sentence — SLOW speed ────────────────────────────────
  Widget _buildNaturalSentence() {
    return LayoutBuilder(
      builder: (context, constraints) {
        final double h = constraints.maxHeight;
              ...
              Flexible(
                child: Center(
                  child: _buildPhraseButton(
                    ...
                    onTap: () => _speakPhraseSlow('I am going home'),
                  ),
                ),
              ),
              SizedBox(height: spacing),
